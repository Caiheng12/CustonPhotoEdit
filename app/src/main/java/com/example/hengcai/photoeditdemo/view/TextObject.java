...
public class TextObject extends ImageObject {
	...
	public void regenerateBitmap() {
		...
		if (srcBm == null){
			srcBm = Bitmap.createBitmap(130, 30, Bitmap.Config.ARGB_8888);
		...
		if(!ValueUtil.isStrEmpty(text)){
			...
			if (textWidth < 1)
				textWidth = 1;
			textBm = Bitmap.createBitmap(textWidth, textSize * (lines.length) + 8,
					Bitmap.Config.ARGB_8888);
			Canvas canvas1 = new Canvas(textBm);
			canvas1.drawColor(Color.TRANSPARENT);
			canvas1.drawARGB(0, 0, 0, 0);
			for (int i = 1; i <= lines.length; i++){
				canvas1.drawText(lines[i - 1], 0, i * textSize, paint);
			}
		}
        setCenter();
	}
	...
	public Typeface getTypefaceObj() {
		Typeface tmptf = Typeface.MONOSPACE;
		if (typeface != null)
		{
