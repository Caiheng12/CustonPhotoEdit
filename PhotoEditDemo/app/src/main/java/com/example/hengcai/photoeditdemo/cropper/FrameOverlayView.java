...
public class FrameOverlayView extends View {
    ...
    private int cropRectWidth=640;//固定宽度
    private float cropMaxHeight=500f;//最大高度
    ...
    private void init() {
        ...
        cornerLength = dpToPx(18);
        cornerLineWidth = dpToPx(3);
    }
    ...
    private void resetFrameRect(int w, int h) {
        ...
        if (shapeType == 1) {
            frameRect.left = (w-cropRectWidth)/2;
            frameRect.top = (int) (h * 0.3);
        } else {
            frameRect.left = (w-cropRectWidth)/2;
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_DOWN: {
                ...
                touchRect.set(event.getX() - radius, event.getY() - radius, event.getX() + radius,
                        event.getY() + radius);
                if(event.getY()>(frameRect.top-radius)&&event.getY()<(frameRect.top+radius)){
                    currentCorner = CORNER_LEFT_TOP;
                    return true;
                }

                if(event.getY()>(frameRect.bottom-radius)&&event.getY()<(frameRect.bottom+radius)){
    ...
    private boolean handleScale(MotionEvent event) {
        switch (currentCorner) {
            case CORNER_LEFT_TOP:
                scaleTo(frameRect.left, event.getY(), frameRect.right, frameRect.bottom);
                return true;
            case CORNER_RIGHT_TOP:
                scaleTo(frameRect.left, event.getY(), frameRect.right, frameRect.bottom);
                return true;
            case CORNER_RIGHT_BOTTOM:
                scaleTo(frameRect.left, frameRect.top, frameRect.right, event.getY());
                return true;
